...
def parse_args():
    parser = argparse.ArgumentParser(description="Run NAIS2.")
    parser.add_argument('--path', nargs='?', default='data/',
                        help='Input data path.')
    parser.add_argument('--dataset', nargs='?', default='ml',
...
class NAIS:
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            if self.algorithm == 0:
                self.embedding_p,self.A,self.EA= self._attention_MLP(
                    self.embedding_q_ * self.embedding_q)
            else:
                n = tf.shape(self.user_input)[1]
                self.embedding_p,self.A,self.EA= self._attention_MLP(tf.concat(
...
if __name__ == '__main__':
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logging.basicConfig(filename=os.path.join(log_dir, "%s_%s_n2:%s_%s" % (
