...
def parse_args():
    parser = argparse.ArgumentParser(description="Run DICF2.")
...
def batch_norm_layer(x, train_phase, name):
    bn_train = tf.layers.batch_normalization(x,momentum=0.99, center=True, scale=True,
        training=True, reuse=None, trainable=True,name=name)

    bn_inference = tf.layers.batch_normalization(x,momentum=0.99, center=True, scale=True,
...
if __name__ == '__main__':
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logging.basicConfig(filename=os.path.join(log_dir, "%s_%s_d2:%s_%s" %(args.dataset, args.embed_size, pret, strftime('%Y-%m-%d-%H:%M:%S', localtime()))), level=logging.INFO)
