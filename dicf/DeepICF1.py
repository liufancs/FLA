...
import argparse
def parse_args():
    parser = argparse.ArgumentParser(description="Run DICF1.")
...
def training(flag, model, dataset, epochs, num_negatives):
    ...
    with tf.Session(config=config) as sess:        # pretrain nor not
        if flag != 0:
        ...
        #fdict = evaluate.init_evaluate_model(model, sess, dataset.testDict, dataset.trainList, dataset.num_users, dataset.num_items)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logging.basicConfig(filename=os.path.join(log_dir, "%s_%s_d1:%s_%s" %(args.dataset, args.embed_size, pret,strftime('%Y-%m-%d-%H:%M:%S', localtime()))), level=logging.INFO)
